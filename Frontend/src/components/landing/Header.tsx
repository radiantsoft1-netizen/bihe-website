...
function getFixedContainingBlockOrigin(element: HTMLElement): { top: number; left: number } {
  ...
  useEffect(() => {
    ...
    const syncSubmenuPosition = () => {
      const item = header.querySelector<HTMLElement>(
        `[data-nav-dropdown="${CSS.escape(openDropdown)}"]`,
      );
      ...
      const trigger = item.querySelector<HTMLElement>(".bihe-site-header__nav-link-group")
        ?? item.querySelector<HTMLElement>(".bihe-site-header__nav-link");
      const rect = (trigger ?? item).getBoundingClientRect();
      const navbar = header.querySelector<HTMLElement>(".bihe-site-header__navbar");
      ...
      const submenuGap = 12;
      const anchorBottom =
        headerScrolled && navbarRect
          ? navbarRect.bottom
          : rect.bottom;
      const isColumns = submenu.classList.contains("bihe-site-header__submenu--columns");
      const estimatedWidth = isColumns
        ? Math.min(29 * 16, window.innerWidth - viewportMargin * 2)
        : 248;
      ...
      submenu.style.setProperty(
        "--submenu-top",
        `${Math.round(viewportTop - containingOrigin.top)}px`,
      );
      submenu.style.setProperty(
        "--submenu-left",
        `${Math.round(left - containingOrigin.left)}px`,
      );
