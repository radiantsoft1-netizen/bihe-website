...
class LogAdminCrudMiddleware
{
    ...
    {
        if (str_starts_with($routeName, 'admin.settings.prospectus.')) {
    ...
    private function refreshWebsiteCache(Request $request): void
    {
        ...
        $extraPaths = $this->extraRevalidatePaths($request);

        if (! $this->websiteRevalidation->revalidateForResource($resource, $extraPaths)) {
            session()->flash(
                'warning',
                'Saved in admin, but the public website cache could not be refreshed. Check NEXTJS_URL, REVALIDATE_SECRET, and Hostinger CDN rules for /api/revalidate.'
            );
        }
    }

    /** @return list<string> */
    private function extraRevalidatePaths(Request $request): array
    {
        $paths = [];

        foreach ($request->route()?->parameters() ?? [] as $parameter) {
            if ($parameter instanceof SitePage && filled($parameter->path)) {
        ...
        if (str_starts_with($routeName, 'admin.research-development-cell.')) {
            $paths[] = '/research/research-and-development-cell';
        }
