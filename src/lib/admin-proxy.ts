...
<body>
  <main>
    <h1>${title}</h1>
    <p>${detail}</p>
    <p><a href="https://bihe-website.vercel.app/">← Back to website</a></p>
  </main>
</body>
</html>`;
}
...
export async function proxyToAdminOrigin(
  ...
  if (!origin) {
    return adminProxyErrorResponse(
      ...
      "Set <code>ADMIN_ORIGIN</code> in Vercel → Project Settings → Environment Variables to your Hostinger Laravel URL, then redeploy.",
    );
  }
  ...
  try {
    ...
    return adminProxyErrorResponse(
      ...
      `The Laravel admin is not reachable at <code>${origin}</code>. Deploy <code>bihe-admin/</code> on Hostinger (subdomain + database), then open this page again.`,
    );
  }
