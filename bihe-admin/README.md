...
To test on the same Wi‑Fi, in `bihe-admin/.env` set:
...
| Site | URL | Host |
|------|-----|------|
| Public website | `https://www.bihe.edu` or Vercel preview | Vercel |
| **Admin panel** | **`https://admin.bihe.edu/admin`** (direct) or **`https://bihe-website.vercel.app/admin`** (proxied via Vercel) | Hostinger + Vercel |

`https://bihe-website.vercel.app/admin` proxies to this Laravel app when Vercel has `ADMIN_ORIGIN=https://admin.bihe.edu` and Hostinger has `ADMIN_PROXY_HOSTS` set (see root `README.md`).

Deploy steps: [docs/HOSTINGER-DEPLOY.md](docs/HOSTINGER-DEPLOY.md) · GitHub Action: `.github/workflows/deploy-bihe-admin.yml`

Production checklist:

1. Subdomain `admin.bihe.edu` created in Hostinger hPanel with DNS active.
2. Document root → `bihe-admin/public` (not the project root).
3. `.env` on server with `APP_URL=https://admin.bihe.edu`, MySQL credentials, `APP_DEBUG=false`.
...
**Quick summary:** upload `bihe-admin/` (FTP or GitHub Actions); document root → `public/`; `.env` on server only; `QUEUE_CONNECTION=sync`, `CACHE_STORE=file` (`.env.example` defaults); Next.js public site deploys separately (Vercel) with `NEXT_PUBLIC_API_URL` pointing at the admin API.
...
Triggers on push to `main` when `bihe-admin/**` changes (or manual **workflow_dispatch**).
...
| `FTP_SERVER_DIR` | `/domains/admin.bihe.edu/public_html/` | Remote folder containing `artisan` (project root, not `public/`) |
...
| Role | Username | Email |
|------|----------|-------|
| Super Admin | `superadmin` | `admin@bihe.edu` |
| Staff | `staff` | `staff@bihe.edu` |
