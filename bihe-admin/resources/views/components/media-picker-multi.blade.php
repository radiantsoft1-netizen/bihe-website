@php
    $pickerId = $id ?? ($name ?? 'images');
    $fileInputName = $name ?? 'images';
    $libraryFieldName = $libraryName ?? 'image_library_paths';
    $existingImages = collect($existingImages ?? []);
    $accept = 'image/jpeg,image/png,image/gif,image/webp';
    $supportsLabel = $hint ?? 'Supports: JPEG, PNG, GIF, WebP — max '.(int) config('uploads.types.image.max_size_kb').' KB each.';
...
<div
    ...
    data-library-field="{{ $libraryFieldName }}"
>
            ...
            <aside class="media-picker__multi-preview" aria-label="Selected images">
                ...
                >
                    @foreach ($existingImages as $image)
                        ...
                        >
                            <img
                                src="{{ \App\Support\StoredFileUrl::publicImage($image->image_path) }}"
                                alt=""
                            ...
                            <input type="hidden" name="keep_image_ids[]" value="{{ $image->id }}">
                        </div>
                    @endforeach
                </div>
    ...
    @error($fileInputName)<p class="form-error">{{ $message }}</p>@enderror
    @error($fileInputName.'.*')<p class="form-error">{{ $message }}</p>@enderror
    @error($libraryFieldName)<p class="form-error">{{ $message }}</p>@enderror
    @error($libraryFieldName.'.*')<p class="form-error">{{ $message }}</p>@enderror
    @error('keep_image_ids')<p class="form-error">{{ $message }}</p>@enderror
    @error('keep_image_ids.*')<p class="form-error">{{ $message }}</p>@enderror
</div>
